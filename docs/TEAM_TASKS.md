...
* [x] BE-024 Public signup endpoint — role `user` hardcoded server-side, `role` in payload ignored; creates + links a **new** `customer` Contact in the same transaction. Never adopts an existing contact by email — that was an account-takeover hole (anyone knowing a customer's email could inherit their invoices); fixed and pinned by `SignupContactIsolationTest`
